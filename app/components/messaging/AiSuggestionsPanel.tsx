  ...
  return (
    <div style={{ background: "rgba(10,25,40,0.97)", border: "1px solid rgba(0,188,212,0.25)", borderRadius: 14, padding: 16, marginBottom: 8, boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}>
      ...
      {mode === "smart_reply" && data.result?.suggestions && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#fff" }}>⚡ Smart Replies</span>
            <button onClick={onDismiss} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", fontSize: 10, cursor: "pointer" }}>✕ Dismiss</button>
          </div>
          {data.result.suggestions.map((s: any, i: number) => {
            ...
            return (
              <div key={i} onClick={() => onUseMessage(s.message)} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "12px 14px", cursor: "pointer", marginBottom: 8, transition: "all 0.15s" }} onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(0,188,212,0.4)"; (e.currentTarget as HTMLElement).style.background = "rgba(0,188,212,0.06)"; }} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(255,255,255,0.08)"; (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.04)"; }}>
                ...
                <div style={{ fontSize: 13, color: "#fff", lineHeight: 1.6, marginTop: 8 }}>{s.message}</div>
              </div>
            );
          })}
        </div>
      ...
      {(mode === "negotiate" || mode === "counter_price") && data.negotiation && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#fff", marginBottom: 8 }}>🤝 Negotiation Advice</div>
          ...
          <div style={{ fontSize: 11, fontStyle: "italic" as const, color: "rgba(207,216,220,0.6)", marginBottom: 8 }}>{data.negotiation.reasoning}</div>
      ...
      {(mode === "professional" || mode === "tone_adjust") && data.result && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#fff", marginBottom: 10 }}>✨ Polished Version</div>
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 3 }}>
              <div style={{ fontSize: 9, color: "rgba(207,216,220,0.5)", marginBottom: 4 }}>Original</div>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.5 }}>{data.result.original || data.result.polished}</div>
            </div>
            <div style={{ flex: 2, borderLeft: "2px solid #00bcd4", paddingLeft: 12 }}>
              <div style={{ fontSize: 9, color: "#00bcd4", marginBottom: 4 }}>Polished ✨</div>
              <div style={{ fontSize: 11, color: "#fff", lineHeight: 1.5 }}>{data.result.polished || data.result.adjusted || ""}</div>
            </div>
          </div>
      ...
      {mode === "summarize" && data.result && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#fff", marginBottom: 8 }}>📝 Conversation Summary</div>
          <div style={{ fontSize: 11, color: "rgba(207,216,220,0.8)", lineHeight: 1.6, marginBottom: 8 }}>{data.result.summary}</div>
